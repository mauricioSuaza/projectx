class UserWorker
	...
	def perform(donation_id)
		...
		if (@user.last_donation_per_month(@padre_1).size > 0)
			...
			if @padre_2
				if @padre_2.total_value_donations_level(1) >= @referral_number
			...
			if @padre_3
				...
				if @padre_3.total_value_donations_level(1) >= @referral_number
			...
			if @padre_4	
				...
				if @padre_4.total_value_donations_level(1) >= @referral_number
			...
			if @padre_5
				...
				if @padre_5.total_value_donations_level(1) >= @referral_number
			...
			if @padre_6
				if @padre_6.total_value_donations_level(1) >= @referral_number
